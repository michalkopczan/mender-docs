...
over the local network, is a Component that the first Pi updates by pushing
commands and Artifact data over MQTT and reading back the results.
...
* The two boards are connected only over the **network**, not a cable.
...
The `mqtt-component` Interface, its `mosquitto` broker configuration, and the
`mender-mqtt-agent` Component agent used on Raspberry Pi #2 all ship in the
...
Install and start the broker itself:
...
(`mender-updated`), set it there on Raspberry Pi #1 so the Interface inherits it:
